...
contract Version3Facet {
    ...
    /// Attention: Only contract owner can set the price. And only today's price can be set.
    ///
    function setPrice(uint year, uint month, uint day, uint price) external {
        LibDiamond.enforceIsContractOwner();
        console.log("Version3Facet.setPrice(%d, %d, %d) called.", year, month, day);
        ...
        (uint todayYear, uint todayMonth, uint todayDay) = BokkyPooBahsDateTimeLibrary.timestampToDate(block.timestamp);
        uint todayTimestamp = BokkyPooBahsDateTimeLibrary.timestampFromDate(todayYear, todayMonth, todayDay);

        require(timestamp == todayTimestamp, 'The price can be set on the same day itself.');
    ...
    function getPrice(uint year, uint month, uint day) external view returns (uint) {
        console.log("Version3Facet.getPrice(%d, %d, %d) called.", year, month, day);
    ...
    function getAvgPrice(uint fromYear, uint fromMonth, uint fromDay, uint toYear, uint toMonth, uint toDay) external view returns (uint) {
        console.log("Version3Facet.getAvgPrice() called.");
